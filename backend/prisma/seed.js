const { PrismaClient, RoleName, InvoiceStatus } = require('@prisma/client');
const bcrypt = require('bcryptjs');

const prisma = new PrismaClient();

async function ensurePayment({ vendorName, amount, currency, paidAt, source, reference }) {
  const existing = await prisma.payment.findFirst({ where: { vendorName, reference: reference ?? null } });
  if (existing) return existing;

  return prisma.payment.create({
    data: { vendorName, amount, currency, paidAt, source, reference }
  });
}
...
async function main() {
  ...
  if (inv) {
    const existingTask = await prisma.reviewTask.findFirst({ where: { invoiceId: inv.id } });
    if (!existingTask) {
      await prisma.reviewTask.create({
        data: { invoiceId: inv.id, status: 'pending', reason: 'Baixa confiança de extração' }
      });
    }
  }

  await ensurePayment({ vendorName: 'AWS', amount: 1200, currency: 'USD', paidAt: new Date('2026-03-29'), source: 'csv', reference: 'CARD-9981' });
  await ensurePayment({ vendorName: 'Microsoft', amount: 840, currency: 'USD', paidAt: new Date('2026-03-30'), source: 'manual', reference: 'TRX-20260330-89' });
}
